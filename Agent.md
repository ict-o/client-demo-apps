...
デモは「本物のシステムを操作している」と商談相手に感じさせること。全文は CLAUDE.md §5-UX。要点:

- 置いたボタン・リンクは**すべて動作させる**。押しても何も起きないUIを1つも残さない（実装しない機能は置かないか disabled 明示）。
- 作成・編集・削除・入力は画面の state に即時反映し、日本語のトースト等で応答する（`alert()` 禁止）。計算は入力に追従して再計算する。
- 検索・絞り込み・並び替えは置いたなら実際に機能させる。飾りのUIは置かない。
- README に書いた実演フローが画面上で最後まで完走できる（行き止まり画面なし）。
- 初期データは業務が進行中に見える内容にし、ヘッダーに架空の担当者名など利用者コンテキストを表示する。
